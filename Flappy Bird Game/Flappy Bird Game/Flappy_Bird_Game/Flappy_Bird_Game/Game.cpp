...
score(0)
{
    ...
    ground_sprite2.setPosition(static_cast<float>(ground_sprite1.getGlobalBounds().width), 578);
    ...
    restart_text.setPosition(150, 630);
    ...
    title_text.setString("BASI-II-B OOP Project-2024");
    sf::FloatRect titleBounds = title_text.getLocalBounds();
    title_text.setPosition((win.getSize().x - titleBounds.width) / 2, -5);

    stitle_text.setFont(font);
    stitle_text.setCharacterSize(18);
    stitle_text.setFillColor(sf::Color::Black);
    stitle_text.setString("Instructor: Ma'am Umme Sadima");
    sf::FloatRect stitleBounds = stitle_text.getLocalBounds();
    stitle_text.setPosition((win.getSize().x - stitleBounds.width) / 2, 30);

    Pipe::loadTextures();

    if (!hit_buffer.loadFromFile("assets/hit_sound.wav"))
    {
    }
...
void Game::startGameLoop()
{
    ...
    while (win.isOpen())
    {
        ...
        while (win.pollEvent(event))
        {
            ...
            {                                                               //Key set to Start the Game!
                if (event.key.code == sf::Keyboard::S && !is_S_pressed)
                {
                    is_S_pressed = true;
                    bird.setShouldFly(true);
                }
                if (event.key.code == sf::Keyboard::Space && is_S_pressed)
                {
                    bird.flapBird(dt);
                }
            }
...
void Game::checkCollisions()
{
    if (!pipes.empty())
    {
            ...
            bird.bird_sprite.getGlobalBounds().top >= 540)
        {
            is_S_pressed = false;
            run_game = false;
            hit_sound.play();                       //Declare function for sound
        }
    }
}
